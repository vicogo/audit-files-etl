"""
Module: compress_aud_files

A module for compressing audit files based on configuration settings into config.properties file.

Source *.aud files to be compressed is defined by local_dir_audit_files configuration parameter.

A result of this task is a file named as {month_str}-aud-files.tar.gz, with mont_str=%Y%m, for example:
    202405-aud-files.tar.gz

This compressed file example contains all may/2024 audit files and is saved into same local_dir_audit_files directory
...
def compress_aud_files(force_month_str =''):
    ...
    compress_imported_aud_files_n_months_after = int (compress_config['compress_imported_aud_files_n_months_after'])
    ...
    log_filename = f'{local_dir_logs}/compress_aud_files_{logFileDateStr}.log'
    ...
    if result == 'OK':
        ...
        month_str = int(fileMonthStr) - compress_imported_aud_files_n_months_after
        if force_month_str != '':
            month_str = force_month_str
        logging.info(f'''Compress files by month: {month_str} {'(Calculated internally)' if force_month_str =='' else ''}''')
        output_filename = f'{destinyDir}/{month_str}_aud_files.tar.gz'
        result = compressFiles(destinyDir, output_filename, month_str, subdirs)
    #
    return result
