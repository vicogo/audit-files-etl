"""
Module: Compress_log_files

A module for compressing log files based on configuration settings into config.properties file.

Source *.aud files to be compressed is defined by local_dir_log_files configuration parameter.

A result of this task is a file named as {month_str}-aud-files.tar.gz, with mont_str=%Y%m, for example:
    202405-log-files.tar.gz

This compressed file example contains all may/2024 log files and is saved into same local_dir_log_files directory

Details of this task are logged into compress_aud_files-{logFileDateStr}.log file
...
Functions:
    1. compress_aud_files(force_month_str='')
        Compresses log files based on configuration settings and optional force_month_str.

Usage Examples:
    1. Compress log files based on default settings:
    >>> from compress_aud_files import compress_aud_files
    >>> compress_aud_files()
    'OK'

    2. Compress log files for a specific month:
    >>> compress_aud_files("202405")
    'OK'

    3. Run the compress_aud_files module:
    >>> from compress_aud_files import main
    >>> main()
    OK
...
def compress_aud_files(force_month_str =''):
    #
    ...
    compress_logs_files_n_months_after = int (compress_config['compress_logs_files_n_months_after'])
    #
    local_server = config['LOCAL_SERVER']
    #destinyDir = local_server['local_dir_log_files']
    #localDirOrganizedByDB = local_server['local_dir_log_files_by_db']
    ...
    log_filename = f'{local_dir_logs}/compress_log_files_{logFileDateStr}.log'
    ...
    #        
    logging.info('======================Compress AUD files ============================')    
    #Compress imported AUD Files 
    if result == 'OK':
        fileMonthStr = fileDate.strftime('%Y%m')
        month_str = int(fileMonthStr) - compress_logs_files_n_months_after
        if force_month_str != '':
            month_str = force_month_str
        logging.info(f'''Compress files by month: {month_str} {'(Calculated internally)' if force_month_str =='' else ''}''')
        output_filename = f'{local_dir_logs}/{month_str}_log_files.tar.gz'
        result = compressFiles(local_dir_logs, output_filename, month_str, subdirs, 'log')
    #
    return result

def main():
    #Call example: python.exe Compress_aud_files.py "202405"
    force_month_str = ''
    for i, arg in enumerate(sys.argv[1:], start=1):
        #  202405
        if i==1:
            force_month_str = arg     
    result = compress_aud_files(force_month_str) 
    print(result)
